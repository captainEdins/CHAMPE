...
import 'package:champe/pages/bottomPages/home/messages/viewModel/viewModel.dart';
...
//send message to the firebase chat
    ...
    try {
      ...
      if (message!.contains('sent')) {
        ...
        if(receiverId == Strings.userGPT){
          ...
          updateVisibility(context: context, typingVisibility: true);
          ...
          sendMessageToChatGpt(messages).then((value) async {
           ...
           updateVisibility(context: context, typingVisibility: false);
          });
  ...
  Future<String?> getOnline(
    ...
    try {
      ...
      await FirebaseFirestore.instance
          .collection('onlineDoc').doc(userId).set({
        'userId': userId,
        'status': false,
        'timestamp': DateTime.now().millisecondsSinceEpoch,
      });
  ...
  //AIzaSyChfZqVpWtF9VZY3zRWfpXJ2BzAK3BHh5I
  Future<String> sendMessageToChatGpt(String message) async {
    ...
    //     "Authorization": "Bearer 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",
    //   },
    ...
    //
    // String reply = parsedReponse['choices'][0]['message']['content'] ?? 'error happened';
    //
    // print(reply);
    // print(parsedReponse);

    final model = GenerativeModel(model: 'gemini-pro', apiKey: 'AIzaSyChfZqVpWtF9VZY3zRWfpXJ2BzAK3BHh5I');
  ...
  //typing
  void updateVisibility({required BuildContext context, required bool typingVisibility}) {
    final visibilityModel = Provider.of<VisibilityModel>(context, listen: false);
    visibilityModel.setVisible(typingVisibility);
  }
  //typing
